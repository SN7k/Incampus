...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [state, setState] = useState<AuthState>(() => {
    ...
    if (savedAuth) {
      try {
        ...
        // MOCK MODE: Always use the first mock user from our mock data
        // This ensures the avatar and other user data are always up-to-date
        // Comment this section out if you want to use real backend
        if (parsedAuth && parsedAuth.isAuthenticated) {
          // Import mockUsers directly to avoid circular dependency
          import('../mock/mockData').then(({ mockUsers }) => {
            if (mockUsers && mockUsers.length > 0) {
              ...
              console.log('AuthContext: Updated user with mock data');
            }
          }).catch(err => {
    ...
    try {
      ...
      const newState = {
        isAuthenticated: true,
            user: mockUser, // Use mock user instead of response data
        loading: false,
        error: null
      };
      ...
      localStorage.setItem('currentPage', 'feed');
          ...
          console.log('AuthContext: Logged in with mock user data');
        }
      }).catch(err => {
        console.error('Error importing mock data:', err);
        
        // Fallback to normal login if mock data import fails
        continueWithNormalLogin(userData, response.data.token);
      });
      // END MOCK MODE
  ...
  // Helper function for normal login flow
  const continueWithNormalLogin = (userData: any, token: string) => {
    ...
    try {
      ...
      import('../mock/mockData').then(({ mockUsers }) => {
        if (mockUsers && mockUsers.length > 0) {
          ...
          console.log('AuthContext: Verified OTP with mock user data');
        }
      }).catch(err => {
        console.error('Error importing mock data:', err);
        
        // Fallback to normal OTP verification if mock data import fails
      // Store token
      tokenService.setToken(response.data.token);
