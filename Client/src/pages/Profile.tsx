...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    if (viewingUserId) {
      ...
      setTimeout(() => {
        // Find the user in mockUsers based on the ID
        // We need to handle different ID formats (string IDs, role-based IDs, etc.)
        let userToView = mockUsers.find(user => user.id === viewingUserId);
        
        // If not found by exact ID, try to find by role-based pattern (e.g., "faculty-1")
        if (!userToView && viewingUserId.includes('-')) {
          const [role, idPart] = viewingUserId.split('-');
          userToView = mockUsers.find(user => 
            user.role === role && 
            (user.id === idPart || parseInt(user.id) === parseInt(idPart))
          );
          
          // If still not found, try to find any user with the matching role
          if (!userToView) {
            userToView = mockUsers.find(user => user.role === role);
          }
        }
        
        console.log('PROFILE DATA: Found user data:', userToView);
        
        if (!userToView) {
          console.error('PROFILE DATA: User not found with ID:', viewingUserId);
          // Fallback to the first user with the same role if possible
          if (viewingUserId.includes('-')) {
            const [role] = viewingUserId.split('-');
            userToView = mockUsers.find(user => user.role === role);
            
            if (userToView) {
              console.log('PROFILE DATA: Using fallback user with role:', role);
            } else {
        ...
        // Create profile data based on the found user
        let mockUserData: ProfileData;
        
        // Determine which profile data to use based on user ID
        if (userToView.id === '3') { // Prof. Robert Williams
          mockUserData = {
            ...
            // Add mock posts for the faculty
            posts: [
              {
                id: 'faculty-post-1',
  ...
  const [profileData, setProfileData] = useState<ProfileData>({
    ...
    bio: user?.bio || 'Computer Science student, class of 2024. Love coding and basketball!',
    education: {
    ...
    achievements: [
      { title: 'Dean\'s List', description: 'For academic excellence', year: '2023' },
      { title: 'Hackathon Winner', description: 'National Coding Challenge', year: '2022' },
      { title: 'Best Student Project', description: 'Department of Computer Science', year: '2021' }
    ],
    interests: ['Programming', 'AI', 'Web Development', 'Music', 'Photography', 'Reading', 'Basketball', 'Chess']
  });
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-300 text-sm md:text-base">{viewingUserId ? viewingUserData?.bio : displayProfile().bio}</p>
                  </div>
                </div>
